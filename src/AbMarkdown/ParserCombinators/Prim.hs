module AbMarkdown.ParserCombinators.Prim
    ( Position(..)
    , Parser()
    , runParser
    , ParserState()
...
withConsumed p = Parser $ \st -> case (evalParser p) st of
    Left err -> Left err
    Right (st', res) ->
...
string s = Parser $ \st -> if s `Text.isPrefixOf` (subject st)
...
runParser p t = fmap snd $ evalParser
    p
    ParserState { subject = t, position = Position 1 1 1, lastChar = Nothing }

runParserWithUnconsumed :: Parser a -> Text -> Either ParseError (a, Text)
runParserWithUnconsumed p t = fmap (\(st, res) -> (res, subject st)) $ evalParser
    p
    ParserState { subject = t, position = Position 1 1 1, lastChar = Nothing }
...
satisfy f = Parser g
  where
    g st = case Text.uncons (subject st) of
        Just (c, _) | f c -> success (advance st (Text.singleton c)) c
        _                 -> failure st "character meeting condition"
{-# INLINE satisfy #-}

-- | Get the next character without consuming it.
peekChar :: Parser (Maybe Char)
peekChar = maybeHead . subject <$> getState where maybeHead = fmap fst . Text.uncons
{-# INLINE peekChar #-}
...
endOfInput = Parser $ \st ->
    if Text.null (subject st) then success st () else failure st "end of input"
{-# INLINE endOfInput #-}
...
takeWhile f =
    Parser $ \st -> let t = Text.takeWhile f (subject st) in success (advance st t) t
{-# INLINE takeWhile #-}

takeWhile1 :: (Char -> Bool) -> Parser Text
takeWhile1 f = Parser $ \st -> case Text.takeWhile f (subject st) of
...
untilTheEnd = Parser $ \st -> success (advance st (subject st)) (subject st)
{-# INLINE untilTheEnd #-}

skip :: (Char -> Bool) -> Parser ()
skip f = Parser $ \st -> case Text.uncons (subject st) of
...
skipWhile f =
    Parser $ \st -> let t' = Text.takeWhile f (subject st) in success (advance st t') ()
{-# INLINE skipWhile #-}

skipWhile1 :: (Char -> Bool) -> Parser ()
skipWhile1 f = Parser $ \st -> case Text.takeWhile f (subject st) of
    t' | Text.null t' -> failure st "characters satisfying condition"
...
scan s0 f = Parser $ go s0 []
  where
    go s cs st = case Text.uncons (subject st) of
        Nothing     -> finish st cs
        Just (c, _) -> case f s c of
            Just s' -> go s' (c : cs) (advance st (Text.singleton c))
            Nothing -> finish st cs
    finish st cs = success st (Text.pack (reverse cs))
{-# INLINE scan #-}

lookAhead :: Parser a -> Parser a
lookAhead p = Parser $ \st ->
    either (const (failure st "lookAhead")) (success st . snd) (evalParser p st)
{-# INLINE lookAhead #-}

notFollowedBy :: Parser a -> Parser ()
notFollowedBy p = Parser $ \st -> either (const (success st ()))
