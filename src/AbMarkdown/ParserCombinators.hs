module AbMarkdown.ParserCombinators
    ( Position(..)
    , Parser
    , ParseError(..)
    , runParser
...
import           AbMarkdown.ParserCombinators.Prim
...
someTill p end = liftA2 (:) p go where go = (end *> pure []) <|> liftA2 (:) p go
{-# INLINE someTill #-}
...
sepBy1 p s = go where go = liftA2 (:) p ((s *> go) <|> pure [])
