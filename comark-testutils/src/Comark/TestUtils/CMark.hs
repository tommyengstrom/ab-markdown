...
nodeToBlock (Node _ (LIST ListAttributes {..}) ns) =
  ...
  where
    listType' = case (listType, listDelim) of
...
nodeToInline (Node _ (LINK d t) ns) =
    Link (Seq.fromList $ map nodeToInline ns) d (mfilter (not . Text.null) $ pure t)
nodeToInline (Node _ (IMAGE d t) ns) =
    Image (Seq.fromList $ map nodeToInline ns) d (mfilter (not . Text.null) $ pure t)
